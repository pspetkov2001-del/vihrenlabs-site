...
I've spent years around ERP programs from the vendor, distribution, and systems-integrator side - and in the post-merger environments where those programs land. From that seat you see the timeline before the customer does: which deals are already staffed, which architects are committed elsewhere, which go-live dates were agreed to in a room where nobody believed them.
...
2. **Master data freeze discipline** - exceptions logged and governance-approved, not verbally waived. Teams that bypass the freeze through informal favors get cutover instability downstream. Every waived exception I've watched come back came back in production, not in test.
